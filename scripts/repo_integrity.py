...
def check(root):
    ...
    for path in markdown:
        ...
        lifecycle = re.search(r'^\\*\\*Lifecycle:\\*\\*\\s*(CANONICAL|DERIVED|HISTORICAL|GUIDANCE|LOG|ARCHIVE)\\b', body, re.M)
        declared = re.search(r'^\\*\\*Lifecycle:\\*\\*\\s*(\\S+)', body, re.M)
        if declared and not lifecycle:
            fail('METADATA', f'{path.relative_to(root)} invalid Lifecycle {declared.group(1)}')
        if lifecycle and lifecycle.group(1) == 'DERIVED':
